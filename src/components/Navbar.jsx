...
const Navbar = () => {
  ...
  return (
    <nav className="w-full bg-blue-600 flex items-center px-4 py-2 gap-4">
      {/* Logo */}
      <Link to="/">
        <img src={WALMART_LOGO} alt="Walmart Logo" className="h-10 w-10 mr-4 cursor-pointer" />
      </Link>
      {/* Pickup/Delivery Pill */}
      <button type="button" onClick={handlePickupToggle} className="flex items-center bg-blue-800 rounded-full px-6 py-2 mr-4 focus:outline-none">
      ...
      <form className="flex-1 flex items-center" onSubmit={handleSearch}>
        ...
        />
        <button type="submit" className="-ml-10 bg-blue-800 rounded-full h-8 w-8 flex items-center justify-center">
